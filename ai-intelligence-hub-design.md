...
> 状态：A1 TrendRadar 在群晖 NAS 四容器生产运行，公网只读日报 `https://trend.shankluo.cc`。生产镜像 **`v2-beta-v4-rc-20260716`**（V4 编辑型 + V4.1；2026-07-16 免费切换与一次付费验收均过，验证页 `/html/2026-07-16/00-25.html`）。直接回滚 `v2-beta-v4-rc-20260715` 或 `v2-beta-rc-20260713`。细节见 roadmap。
>
...
当前状态：历史与数据策划见 [`docs/superpowers/specs/2026-07-09-v2-beta-intelligence-history-ui-design.md`](docs/superpowers/specs/2026-07-09-v2-beta-intelligence-history-ui-design.md)；深空研判报告视觉见 [`docs/superpowers/specs/2026-07-10-v2-beta-deep-space-report-ui-design.md`](docs/superpowers/specs/2026-07-10-v2-beta-deep-space-report-ui-design.md)。Gate 9.1 已完成：受控 `history.json`、`history.html`、历史抽屉、深空报告和无障碍加固已通过本地免费回归与固定 `linux/amd64` RC 容器验证；冻结 RC 为 `xjiankong-trendradar:v2-beta-rc-20260713`。Gate 10 采用“固定 RC 镜像 + `/history.json`、`/history.html` 两个 Nginx 精确白名单”的最小生产变更，拆为 10A—10D 独立闸门；其执行进度与待批准闸门见 [`docs/roadmap.md`](docs/roadmap.md)。
...
- [群晖 NAS 部署设计（已归档 · 早期三容器快照）](docs/archive/2026-07-03-nas-deployment-design.md)
- [群晖 NAS 实施计划（已归档 · 早期三容器快照）](docs/archive/2026-07-04-nas-deployment.md)

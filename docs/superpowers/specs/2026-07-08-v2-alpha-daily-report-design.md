...
> 状态：本地验证通过（2026-07-08）。生产同步未执行，需另起计划。
> 设计日期：2026-07-08。
>
...
```
┌─────────────────────────────────────┐
│ 01 今日核心判断（全宽摘要卡片）       │  ← 渐变背景，突出
└─────────────────────────────────────┘
┌──────────────────┐ ┌──────────────────┐
│ 02 研究/模型突破   │ │ 03 产品与工具变化 │
└──────────────────┘ └──────────────────┘
┌──────────────────┐ ┌──────────────────┐
│ 04 开源项目生态    │ │ 05 大厂/融资/政策 │  ← 2 列网格
└──────────────────┘ └──────────────────┘
┌──────────────────┐ ┌──────────────────┐
│ 06 X 关键人物观点  │ │ 07 风险与下周关注 │
└──────────────────┘ └──────────────────┘
┌─────────────────────────────────────┐
│ 08 原始来源索引（可折叠 details）     │  ← 紧凑脚注
└─────────────────────────────────────┘
```

### 6.3 新增 CSS

- `.ai-executive-summary`：全宽摘要卡片（渐变背景，粗体标题，大字号）
- `.ai-judgment-card`：2-7 号面板卡片
- `.ai-judgment-card.empty`：灰化"暂无高置信信号"样式
- `.ai-source-index`：可折叠来源索引
- 暗色模式适配
...
| `report/html.py` | 修改 | `default_region_order` + 新增 v2 CSS 类 |

...
>
> 状态：**已完成（2026-07-08）**。Task 1-8 全部通过，本地 v2-alpha 验证完成；UI 视觉收口完成。生产同步未执行，需按独立计划逐闸门确认。
>
...
生产同步（NAS 文件上传 + 手动触发 + 公网验收）另起独立实施计划。生产同步基线必须使用 TrendRadar `v2-alpha` HEAD `33d80973`，不能再使用旧基线 `20575e25`。
...
1. **先不要部署。**
2. 让另一个 agent 继续执行生产同步计划中的「只读核验闸门」：
   - `docker ps`
   - `docker inspect xjiankong-trendradar` mounts
   - 拉取 NAS 当前 `config.yaml` 到本地 `/tmp`
   - 与 TrendRadar `v2-alpha` HEAD `33d80973` 的 `config/config.yaml` 做 diff
3. 只读核验输出后，再决定是否放行：
   - 备份 + 上传配置文件
   - Python 源码同步策略
   - 容器重建
...
- 另一个 agent 负责生产同步计划执行与 NAS 事实核验。
- Codex 当前负责方案审查、UI 本地修改、文档基线维护和闸门把关。
- 每个生产闸门必须单独回报结果并等待老板确认；不得把只读核验、上传、重启、AI 触发、公网验证合并执行。
- 另一个 agent 后续引用的 TrendRadar 基线必须是 `33d80973`。

...
5. **当前唯一下一闸门：Gate 3 原始消息链接 404 只读诊断。** 由 glm5.2 只读抽样现有报告的 RSS、X/Nitter、热榜与来源索引链接，分类确认 URL 生成错误、Nitter 不可用、源站删除、登录限制或历史快照链接问题；不改代码、不访问 NAS、不重启容器、不触发 AI。完成后必须停止并提交事实报告，交由 Codex 审计。
6. AI HOT 风格「过去 24 小时精选情报」展示层保留为 v2-beta 证据层参考，不替代 8 板块研判框架。
7. 后续内容板块预留「主流模型官方价格动态」：跟踪 Claude、OpenAI GPT、Gemini、Grok、智谱 GLM/GLM 5.2、DeepSeek、通义千问、GPT Image 2、字节 Seedream 等模型的官方降价、活动、价格幅度和横向价格优势。价格数据必须来自官方价格页、公告或开发者文档；不使用二手传闻，不在第一轮历史 UI mockup 中实现。
8. 后续内容板块预留「AI 创业方向与 AIGC 应用风向」：重点跟踪漫剧、AI 小说、AI 音乐以及新的 AI 创业方向。需要分析各板块趋势、平台反应、商业化机会和可核验数据；新闻热度不能直接等同于创业机会，不在第一轮历史 UI mockup 中实现。
9. 项目终极目标升级为老板个人的「AI 情报收集中心」：覆盖 AI 基础技术、AI 应用技术、AIGC 创业方向、模型成本、平台政策和商业化机会。v2-beta 第一阶段仍先做历史 UI 与情报终端框架，不一次性引入所有新数据源。

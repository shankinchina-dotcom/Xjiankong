...
3. v2-beta 已确定为「AI 情报终端与历史版本浏览」：产品与数据边界见 [`../specs/2026-07-09-v2-beta-intelligence-history-ui-design.md`](../specs/2026-07-09-v2-beta-intelligence-history-ui-design.md)，视觉以 [`../specs/2026-07-10-v2-beta-deep-space-report-ui-design.md`](../specs/2026-07-10-v2-beta-deep-space-report-ui-design.md) 为准。
4. Gate 2 历史 UI mockup 和 Gate 2.5 深空研判报告视觉设计均已完成；效果图见 [`../specs/v2-beta-deep-space-report-reference.png`](../specs/v2-beta-deep-space-report-reference.png)，仅作风格参考，不含真实数据。
5. Gate 3 原始消息链接 404 只读诊断已完成并经 Codex 审计：已复现一条华尔街见闻源站 404；Nitter/X 与 GitHub 样本可达，知乎样本为受限 403；未发现本地 HTML 路由问题；当前普通热榜渲染与 `source_map` 都采用 `url → mobileUrl`。结论是需要来源 fallback／不可用提示策略，不是修本地路由。
6. Gate 4 Codex 审计 mockup 与数据映射已完成：现有 8 板块、`SourceRef`、生成时间和统计可直接复用；历史版本时间线必须由 `history.json` 提供，板块一句总结第一轮使用静态引导语，来源状态按确定性规则呈现；网页改造须隔离 email renderer。
7. Gate 5 受控 `history.json` 生成已在隔离分支 `codex/v2-beta-history` 完成，提交 `c6525e1f` 与 `4caab5d0`：只扫描合法历史快照、输出公开路径与真实时间、排除内部文件和无效日期／时间；fixture 与 v2-alpha 回归通过，产物未提交。该代码尚未合并回 `v2-alpha`，更未同步生产。
8. Gate 6 深空报告与历史 UI 本地实现、Gate 7 免费独立审计和 Gate 8 RC 冻结均已完成；发布候选为 `codex/v2-beta-rc` / `8f7e385a`，尚未同步生产。
9. **当前唯一下一闸门：Gate 9 本地 RC 镜像构建与一次性容器验证。** 当前阻塞于 Docker Engine／BuildKit 的 GHCR 与 Docker Hub metadata／pull 通道；`uv` stage cache 已验证，Python base cache 首次导入失败并留下 714B 无效标签。下一步只按受控恢复路径处理本地 cache；Gate 9 完成前不得上传镜像、访问 NAS、修改 `.env`、重启容器或进入生产同步。
10. AI HOT 风格「过去 24 小时精选情报」展示层保留为 v2-beta 证据层参考，不替代 8 板块研判框架。
11. 后续内容板块预留「主流模型官方价格动态」：跟踪 Claude、OpenAI GPT、Gemini、Grok、智谱 GLM/GLM 5.2、DeepSeek、通义千问、GPT Image 2、字节 Seedream 等模型的官方降价、活动、价格幅度和横向价格优势。价格数据必须来自官方价格页、公告或开发者文档；不使用二手传闻，不在第一轮历史 UI mockup 中实现。
12. 后续内容板块预留「AI 创业方向与 AIGC 应用风向」：重点跟踪漫剧、AI 小说、AI 音乐以及新的 AI 创业方向。需要分析各板块趋势、平台反应、商业化机会和可核验数据；新闻热度不能直接等同于创业机会，不在第一轮历史 UI mockup 中实现。
13. 项目终极目标升级为老板个人的「AI 情报收集中心」：覆盖 AI 基础技术、AI 应用技术、AIGC 创业方向、模型成本、平台政策和商业化机会。v2-beta 第一阶段仍先做历史 UI 与情报终端框架，不一次性引入所有新数据源。

...
> 状态：RSS 源已接入运行中的 TrendRadar；项目发现和版本提醒可用，star velocity、commit 频率等指标能力仍待后续补齐。
> 当前 TrendRadar 只保留标准 RSS 字段，并按标题过滤；feed 中的 `stars/addStars` 自定义字段尚未进入存储和日报。
...
**实施范围**：已修改 TrendRadar 的 `config/config.yaml` 和 Xjiankong 的 `config/trendradar/frequency_words.txt`，未增加自定义采集代码。
...
- 运行中的 TrendRadar 已用 isboyjc 替换 mshibanami。
- `config/trendradar/rss-feeds.yaml` 受控快照尚未完整同步新增源，后续应单独处理；本轮不修改 RSS 配置。
...
2. **Star 增长趋势追踪**：isboyjc feed 含 `addStars`，但当前 TrendRadar 解析器不会保存该自定义字段，因此需求尚未满足；后续需单独设计指标 enrichment
